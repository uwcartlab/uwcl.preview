...
class Ridizain_Ephemera_Widget extends WP_Widget {
	 ...
	 * @since Ridizain 1.0
	 *
	 * @return Ridizain_Ephemera_Widget
	 */
	public function __construct() {
		parent::__construct( 'widget_ridizain_ephemera', __( 'Ridizain Ephemera', 'ridizain' ), array(
			'classname'   => 'widget_ridizain_ephemera',
			'description' => __( 'Use this widget to list your recent Aside, Quote, Video, Audio, Image, Gallery, and Link posts', 'ridizain' ),
		) );
		...
		$this->format_strings = array(
			'aside'   => __( 'Asides',    'ridizain' ),
			'image'   => __( 'Images',    'ridizain' ),
			'video'   => __( 'Videos',    'ridizain' ),
			'audio'   => __( 'Audio',     'ridizain' ),
			'quote'   => __( 'Quotes',    'ridizain' ),
			'link'    => __( 'Links',     'ridizain' ),
			'gallery' => __( 'Galleries', 'ridizain' ),
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $ephemera->have_posts() ) :
				...
				<article <?php post_class(); ?>>
					<div class="entry-content">
						...
						<p class="wp-caption-text">
							<?php
								printf( _n( 'This gallery contains <a href="%1$s" rel="bookmark">%2$s photo</a>.', 'This gallery contains <a href="%1$s" rel="bookmark">%2$s photos</a>.', $total_images, 'ridizain' ),
			...
			<a class="post-format-archive-link" href="<?php echo esc_url( get_post_format_link( $format ) ); ?>"><?php printf( __( 'More %s <span class="meta-nav">&rarr;</span>', 'ridizain' ), $this->format_strings[ $format ] ); ?></a>
			<?php
	...
	function form( $instance ) {
		...
		?>
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'ridizain' ); ?></label>
			<input id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>"></p>

			<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Number of posts to show:', 'ridizain' ); ?></label>
			<input id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="text" value="<?php echo esc_attr( $number ); ?>" size="3"></p>

			<p><label for="<?php echo esc_attr( $this->get_field_id( 'format' ) ); ?>"><?php _e( 'Post format to show:', 'ridizain' ); ?></label>
...
class Ridizain_RecentPostWidget extends WP_Widget {
    /** constructor */

	function Ridizain_RecentPostWidget() {
		$widget_ops = array(
		  'classname' => 'widget_ridizain_recent_posts', 
		  'description' => __('Ridizain - Recent Posts (Widescope Thumbs). Can be used for the sidebar and as an alternative to the Showcase Widget.','ridizain') );
		$this->WP_Widget(
		  'ridizain-recent-posts', __('Ridizain - Recent Posts','ridizain'), $widget_ops);
	}
  ...
  /** @see WP_Widget::widget */
    function widget($args, $instance) {		
        extract( $args );
        $title = apply_filters('widget_title', empty($instance['title']) ? __('Ridizain Recent Posts','ridizain') : $instance['title']);
                  ...
                  <?php } ?>      
                                ...
                                <ul class="ridizain-recent-post-widget">
                                    ...
                                    <li>
                                        ...
                                        $cf_thumb = (isset($custom["ridizain_thumb"][0]))? $custom["ridizain_thumb"][0] : "";
                                        
                                        if($cf_thumb!=""){
                                            $thumb = '<img src='. $cf_thumb .' alt="" width="77" height="77" class="frame"/>';
                                        }elseif(has_post_thumbnail($post->ID) ){
                                            $thumb = get_the_post_thumbnail($post->ID, 'ridizain-recentpost-thumb', array('class' => 'frame'));
                                        }else{
                                            $thumb ="";
                                        }
                                        echo  $thumb;
										...
										<p><?php echo ridizain_recentpost_excerpt(); ?></p>
										<?php } ?>
    ...
    function form($instance) {
        ...
        ?>
            <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Widget Title:', 'ridizain'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label></p>
			
            <p><label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Select A Category:', 'ridizain'); ?><br />
			<?php 
			$args = array(
			'selected'         => $category,
			'echo'             => 1,
            ...
            <p><label for="<?php echo $this->get_field_id('showpost'); ?>"><?php _e('Number of Post to show:', 'ridizain'); ?> <input class="widefat" id="<?php echo $this->get_field_id('showpost'); ?>" name="<?php echo $this->get_field_name('showpost'); ?>" type="text" value="<?php echo $showpost; ?>" /></label></p>
            
			<p><label for="<?php echo $this->get_field_id('disabletitle'); ?>"><?php _e('Hide Widget Title:', 'ridizain'); ?> 
            ...
            <p><label for="<?php echo $this->get_field_id('disablethumb'); ?>"><?php _e('Hide Thumbnail:', 'ridizain'); ?> 
            ...
            <p><label for="<?php echo $this->get_field_id('disabledate'); ?>"><?php _e('Hide Post Date:', 'ridizain'); ?> 
			...
			<p><label for="<?php echo $this->get_field_id('disableexcerpt'); ?>"><?php _e('Hide the Excerpt:', 'ridizain'); ?>
			...
			<p><label for="<?php echo $this->get_field_id('disablereadmore'); ?>"><?php _e('Hide Readmore Button:', 'ridizain'); ?>
