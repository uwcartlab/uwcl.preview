...
function ridizain_customize_register( $wp_customize ) {
	...
	$wp_customize->get_control( 'header_textcolor' )->label = __( 'Site Title &amp; Tagline Color', 'ridizain' );
    ...
    $wp_customize->add_section( 'ridizain_theme_notes' , array(
		...
		<br/><br/><a href="%2$s" target="_blank" />View Theme Demo</a> | <a href="%3$s" target="_blank" />Get Theme Support</a> ', 'ridizain' ), admin_url( '/nav-menus.php' ), esc_url('http://demo1.ridizain.com/'), esc_url('http://wordpress.org/support/theme/ridizain') ),
		'priority'   => 30,
    ) );
	...
	$wp_customize->add_section( 'ridizain_content_options' , array(
       'title'      => __('Ridizain Content Options','ridizain'),
	   'description' => sprintf( __( 'Use the following settings to set content options.', 'ridizain' )),
       'priority'   => 32,
    ) );
    ...
    $wp_customize->add_setting('ridizain_feed_cat', array(
		'default'           => 'All Categories',
		'sanitize_callback' => 'ridizain_sanitize_feed_categories',
	));
	$wp_customize->add_control( 'ridizain_feed_cat', array(
		'settings' => 'ridizain_feed_cat',
		'label'   => __('Select Blog Feed Category:', 'ridizain'),
		'section'  => 'ridizain_content_options',
		'priority' => 1,
		'type'    => 'select',
		'choices' => ridizain_feed_categories_choices(),
	));
	
	$wp_customize->add_setting( 'ridizain_feed_excerpt_length', array(
	    'default' => '85',
		'sanitize_callback' => 'ridizain_sanitize_integer',
    ));
	...
	$wp_customize->add_setting(
    'ridizain_recentpost_excerpt_length', array(
	    'default' => '15',
		'sanitize_callback' => 'ridizain_sanitize_integer',
    ));
	...
	$wp_customize->add_setting(
        'featured_content_boxed', array (
			'sanitize_callback' => 'ridizain_sanitize_checkbox',
		)
    );

    $wp_customize->add_control(
        'featured_content_boxed',
    array(
        'type'     => 'checkbox',
        'label'    => __('Give Featured Content The Boxed Look? - Off The Edges!', 'ridizain'),
        'section'  => 'featured_content',
		'priority' => 2,
        )
    );
	...
	$wp_customize->add_setting('featured_content_custom_type', array(
	    'default'  => 'post',
		'sanitize_callback' => 'ridizain_sanitize_featured_type',
	));
	...
	$wp_customize->add_control( 'featured_content_custom_type', array(
	    ...
	    'priority' => 4,
	    'type'    => 'select',
	    'choices' => ridizain_featured_content_type(),
	));
	...
	$wp_customize->add_setting(
        'ridizain_featured_excerpt_visibility', array (
			'sanitize_callback' => 'ridizain_sanitize_checkbox',
		)
    );

    $wp_customize->add_control(
        'ridizain_featured_excerpt_visibility',
    array(
        'type' => 'checkbox',
	...
	$wp_customize->add_setting('ridizain_title_hover', array(
        'default'           => '03c1bb',
        'sanitize_callback' => 'sanitize_hex_color',
        'capability'        => 'edit_theme_options',
    ));
 
    $wp_customize->add_control( new WP_Customize_Color_Control($wp_customize, 'ridizain_title_hover', array(
        'label'    => __('Site Title Hover Color', 'ridizain'),
        'section'  => 'colors',
		'priority' => 3,
        'settings' => 'ridizain_title_hover',
    )));
	...
	// Theme Notes section
	$wp_customize->add_setting( 'ridizain_theme_notice', array(
...
function ridizain_feed_categories_choices() {
...
function ridizain_sanitize_feed_categories($cats) {
...
function ridizain_featured_content_type() {
...
function ridizain_sanitize_featured_type($cpt) {
...
function ridizain_accent_css(){
  ...
  $title_hover        = get_theme_mod( 'ridizain_title_hover' );
  ...
  $title_hover         = '#' . str_replace( '#', '', $title_hover );
  ...
  // Apply settings to relevant elements  ?> 
    <style>
		...
		.site-title a:hover {
	        color: <?php echo $title_hover; ?> !important;
        }
		.site-navigation a {
