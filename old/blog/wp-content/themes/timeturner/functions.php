...
$timeturner_themever = "1.0.15";										//Theme version
...
function timeturner_setup() {
  ...
  add_image_size( 'fullwidth-thumb', 990, 9999 );
...
function timeturner_scripts_styles() {
  ...
  // Loads additional stylesheet for IE for correct displaying in RTL.
    wp_enqueue_style( 'timeturner-style-ie-rtl', get_template_directory_uri() . '/css/style-ie-rtl.css' );
  // Apply IE conditionals
    $GLOBALS['wp_styles']->add_data( 'timeturner-style-ie', 'conditional', 'lte IE 8' );
    $GLOBALS['wp_styles']->add_data( 'timeturner-style-ie-rtl', 'conditional', 'IE' );
}
...
<?php $timeturner_previous_post = get_adjacent_post( false, "", true );
$timeturner_next_post = get_adjacent_post( false, "", false ); ?>
<div id="<?php echo $nav_id; ?>" class="navigation" role="navigation">
  <h3 class="navigation-headline section-heading"><?php _e( 'Post navigation', 'timeturner' ); ?></h3>
<?php if ( !empty($timeturner_previous_post) ) { ?>
	<p class="nav-previous"><a href="<?php echo esc_url(get_permalink($timeturner_previous_post->ID)); ?>" title="<?php echo esc_attr($timeturner_previous_post->post_title); ?>"><?php _e( '&larr; Previous post', 'timeturner' ); ?></a></p>
<?php } if ( !empty($timeturner_next_post) ) { ?>
	<p class="nav-next"><a href="<?php echo esc_url(get_permalink($timeturner_next_post->ID)); ?>" title="<?php echo esc_attr($timeturner_next_post->post_title); ?>"><?php _e( 'Next post &rarr;', 'timeturner' ); ?></a></p>
<?php } ?>
</div>
<?php } 
...
$config = array(
    ...
    'menu'         => 'install-my-theme-plugins',
		'strings'      	 => array(
		'page_title'             => __( 'Install Recommended Plugins', 'timeturner' ),
